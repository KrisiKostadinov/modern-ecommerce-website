...
import updateNameAction from "@/app/dashboard/products/[slug]/_actions/update-name";
...
export default function UpdateName({ id, name }: UpdateNameProps) {
  ...
  const onSubmit = async (values: FormSchemaProps) => {
    ...
    if (result.createdProduct) {
      return router.push(`/dashboard/products/${result.createdProduct.id}`);
    }
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Input
                      placeholder="Напишете име на продукта"
                      {...field}
